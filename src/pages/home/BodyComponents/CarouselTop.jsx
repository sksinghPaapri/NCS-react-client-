...
export default function CarouselTop() {
  ...
  const ButtonGroup = ({ next, previous, goToSlide, ...rest }) => {
    ...
    return (
      <div
        className="absolute top-20 md:top-56 lg:top-96 mb-4 gap-4 flex justify-between 
        items-center w-full"
      >
        <button
          className="block p-3 bg-slate-300 ms-0 md:ms-10 lg:ms-20 rounded-full hover:bg-slate-400"
          onClick={() => previous()}
        >
          {" "}
          <FiChevronLeft />
        ...
        <button onClick={() => next()}>
          <span className="block p-3 bg-slate-300 me-0 md:me-10 lg:me-20 rounded-full hover:bg-slate-400">
            <BiChevronRight />
          </span>
        </button>
      </div>
